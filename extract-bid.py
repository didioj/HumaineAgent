...
def interpretMessage(watsonResponse):
    ...
    print("- intents from watsonResponse:", watsonResponse['intents'])
    print("- entities from watsonResponse:", watsonResponse['entities'])
    ...
    if intents and intents[0]['intent'] == "Offer" and intents[0]['confidence'] > 0.2:
        ...
        if extractedOffer['price']:
            ...
            if watsonResponse['input']['role'] == 'buyer':
                ...
                cmd['type'] = "SellOffer"
                # change type to BulkOffer and remove quantity if 'bulk' is in keyword
                for keyword in keywords:
                    if 'discounted' in keyword:
                        cmd['type'] = "BulkOffer"
                        if 'quantity' in cmd:
                            cmd.pop('quantity')
        else:
    ...
    elif intents and intents[0]['intent'] == "BulkOffer" and intents[0]['confidence'] > 0.2:
        extractedOffer = extractOfferFromEntities(entities)
        cmd = {
            # 'quantity': extractedOffer['quantity'],
            'type': "BulkOffer"
